/**
 * Catan — Frontend Renderer
 * Simplified Catan with hex board canvas + resource panel.
 */
window.GameRenderer = (() => {
  ...
  // Hex grid layout (19 tiles in standard Catan)
  const HEX_LAYOUT = [
    [0,1,2],
    [3,4,5,6],
  ...
  function init(_session, _sendMove) {
    ...
    document.getElementById('gameWrap').innerHTML = `
      <div class="catan-wrap">
        <canvas id="catanCanvas" class="catan-canvas"></canvas>
        <div id="catanInfo" style="text-align:center;font-family:var(--font-head);font-size:14px;color:var(--accent);margin:6px 0"></div>
        <div id="diceDisplay" style="text-align:center;font-size:36px;margin:4px 0"></div>
        <div class="catan-sidebar" id="catanResources"></div>
        <div class="catan-actions" id="catanActions"></div>
      </div>
    `;
  ...
  function renderResources() {
    const wrap = document.getElementById('catanResources');
    if (!myPlayer) return;
  ...
  function renderActions() {
    const bar = document.getElementById('catanActions');
  ...
  function renderInfo() {
    const el = document.getElementById('catanInfo');
    if (!state) return;
    const isMyTurn = state.currentTurn === session.playerId;
