/**
 * Snake & Ladder — Frontend Renderer
 * Full 10×10 board canvas with snakes and ladders drawn.
 */
window.GameRenderer = (() => {
  ...
  function init(_session, _sendMove) {
    ...
    document.getElementById('gameWrap').innerHTML = `
      <div style="display:flex;flex-direction:column;align-items:center;gap:14px">
        <div id="salEvent" class="event-banner" style="display:none"></div>
        <canvas id="salCanvas" class="sal-canvas" title="Click to roll dice"></canvas>
        <div id="salDice" class="dice-display" title="Click to roll your turn">🎲</div>
        <div id="salInfo" style="font-family:var(--font-head);font-size:14px;color:var(--accent);text-align:center;letter-spacing:1px"></div>
        <div id="salScores" style="display:flex;gap:12px;flex-wrap:wrap;justify-content:center"></div>
      </div>
    `;

    canvas = document.getElementById('salCanvas');
    ctx    = canvas.getContext('2d');
    canvas.width = canvas.height = SIZE + PAD*2;
    canvas.style.maxWidth = '100%';
    canvas.addEventListener('click', onRoll);

    document.getElementById('salDice').addEventListener('click', onRoll);
  ...
  function renderDice() {
    const el    = document.getElementById('salDice');
    const faces = ['','⚀','⚁','⚂','⚃','⚄','⚅'];
  ...
  function renderInfo() {
    const el  = document.getElementById('salInfo');
    const myTurn = state?.currentTurn === session.playerId;
  ...
  function renderScores() {
    const wrap = document.getElementById('salScores');
    if (!state) return;
  ...
  function renderEvent() {
    const el  = document.getElementById('salEvent');
