/**
 * Ludo — Frontend Renderer
 * Full canvas Ludo board with dice rolling.
 */
window.GameRenderer = (() => {
  ...
  function init(_session, _sendMove) {
    ...
    document.getElementById('gameWrap').innerHTML = `
      <div style="display:flex;flex-direction:column;align-items:center;gap:12px">
        <canvas id="ludoCanvas" class="ludo-canvas"></canvas>
        <div id="ludoDice" class="dice-display" title="Click to roll">🎲</div>
        <div id="ludoInfo" style="font-family:var(--font-head);font-size:14px;color:var(--accent);text-align:center;letter-spacing:1px"></div>
        <div id="ludoPieces" style="display:flex;gap:8px;flex-wrap:wrap;justify-content:center"></div>
      </div>
    `;
    ...
    document.getElementById('ludoDice').addEventListener('click', onDiceClick);
    drawBoard();
  }
  ...
  function renderDice() {
    const el = document.getElementById('ludoDice');
    const faces = ['', '⚀','⚁','⚂','⚃','⚄','⚅'];
  ...
  function renderInfo() {
    const el = document.getElementById('ludoInfo');
    if (!state) return;
    const isMyTurn = state.currentTurn === session.playerId;
  ...
  function renderPieceButtons() {
    const wrap = document.getElementById('ludoPieces');
